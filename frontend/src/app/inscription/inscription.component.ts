...
export class InscriptionComponent implements OnInit {
  ...
  onSubmitInscription(form: NgForm) {
    ...
    if (form.value.password === form.value.passwordConfirmation) {
      ...
      this.httpClient
        .post('http://localhost:3000/inscription', this.formData)
        .subscribe(
          (response: any) => {
            alert(
              `Inscription réussie ! Votre compte est bien créé ${form.value.firstname} ${form.value.lastname}.`
            );
      ...
      alert(
        'Erreur : Le mot de passe et la confirmation du mot de passe sont différents !'
      );
    }
  }
